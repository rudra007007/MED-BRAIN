...
export default function Menubar({ onClose }: MenubarProps) {
  ...
  const renderPreferencesSettings = () => (
    <View style={styles.settingsDetailContainer}>
      ...
      <View style={[styles.dropdown, { backgroundColor: colors.backgroundCard }]}>
        <TouchableOpacity
          style={styles.dropdownTrigger}
          onPress={() => setShowThemeDropdown(!showThemeDropdown)}
        >
          <View style={styles.optionRow}>
            {theme === 'light' ? <Sun size={20} color={colors.accent} /> :
             theme === 'dark' ? <Moon size={20} color={colors.accent} /> :
             <Accessibility size={20} color={colors.accent} />}
            <Text style={[styles.dropdownValue, { color: colors.text }]}>
              {theme.charAt(0).toUpperCase() + theme.slice(1)}
            </Text>
          </View>
          <ChevronLeft size={20} color={colors.text} style={{ transform: [{ rotate: showThemeDropdown ? '-90deg' : '90deg' }] }} />
        </TouchableOpacity>
        {showThemeDropdown && (
            ...
            {['light', 'dark', 'system'].map((t) => (
              ...
              >
                <View style={styles.optionRow}>
                  ...
                  <Text style={[styles.dropdownItemText, { color: theme === t ? colors.accent : colors.text }]}>
